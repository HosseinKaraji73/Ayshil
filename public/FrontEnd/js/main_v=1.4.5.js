...
function productLightbox() {
    if ($('.product-image').length && $('.product-thumbnail').length) {
        $('.product-gallery-thumbnail').click(function (e) {
            ...
            $('#product-gallery__current img').attr('src', $(this).attr('data-src'));
        });
        ...
        $('.product-image').click(function (e) {
            ...
            var currentImageEl = document.querySelector('#product-gallery__current div');
            if (!$('.pinch-zoom-container').length && window.innerWidth < 1024) {
                var currentImagePinch = new PinchZoom.default(currentImageEl, {
